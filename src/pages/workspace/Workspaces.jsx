...
function WorkspaceCard({ workspace, onEdit, onDelete, delay }) {
  ...
  return (
    <motion.div
      ...
      className="card p-5 group relative cursor-pointer"
    ...
    >
      ...
      <div className="flex items-start justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="font-semibold text-surface-900 leading-tight">{workspace.name}</h3>
            <div className="flex items-center gap-1.5 mt-0.5">
              {workspace.isOwner && (
                <span className="flex items-center gap-0.5 text-[10px] text-yellow-600 font-semibold">
                  <Crown size={10} />Owner
                </span>
              )}
              <Badge color={PLAN_CONFIG[workspace.plan]?.color || 'gray'} size="xs">
                {workspace.plan}
              </Badge>
            </div>
          </div>
      ...
      {/* Description */}
      <p className="text-sm text-surface-500 mb-4 leading-relaxed line-clamp-2">
      ...
      <div className="flex items-center gap-4 mb-4 text-sm text-surface-600">
        <div className="flex items-center gap-1.5">
          <FolderKanban size={14} className="text-surface-400" />
      ...
      <div
        ...
        style={{ background: `linear-gradient(90deg, ${workspace.color}, transparent)` }}
      />
    </motion.div>
  );
}
    ...
    <PageTransition className="p-6 max-w-[1400px] mx-auto">
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-surface-500 text-sm mt-1">{workspaces.length} workspaces across your organization</p>
        </div>
      ...
      {filtered.length === 0 ? (
        <EmptyState
          ...
          description={search ? 'Try a different search term.' : 'Create your first workspace to organize your team and projects.'}
      ...
      <Modal isOpen={deleteModal.isOpen} onClose={deleteModal.close} title="Delete Workspace" size="sm">
        <div className="text-center">
          ...
          <p className="text-sm text-surface-500 mb-6">This action cannot be undone. All projects and data within this workspace will be permanently deleted.</p>
